...
public abstract class OperationUI extends JPanel implements IDomainObjectUI, IOperationListener {
	...
	public OperationUI(String parent, String scope, String id) {
		...
		initSignal();
		Services.getService().getProgramData().addOperationListener(this);
	}
	...
	public boolean isBothContentSet() {
		String ref1 = "", ref2 = "";
		if (Services.getService().getModelMapping().get(expressionBaseUI_1.getCurrentModelID()) instanceof IVPVariableReference) {
			ref1 = ((IVPVariableReference) Services.getService().getModelMapping().get(expressionBaseUI_1.getCurrentModelID()))
			        .getReferencedID();
		}
		if (Services.getService().getModelMapping().get(expressionBaseUI_2.getCurrentModelID()) instanceof IVPVariableReference) {
			ref2 = ((IVPVariableReference) Services.getService().getModelMapping().get(expressionBaseUI_2.getCurrentModelID()))
			        .getReferencedID();
		}
