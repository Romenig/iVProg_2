...
public class ExpressionHolderUI extends JPanel implements IExpressionListener {
	...
	private void init(String parent, String scopeID) {
		...
		currentModelID = "";
		setOperationContext("");
		Services.getService().getProgramData().getExpressionListeners().add(this);
	}
		...
		Action changeToConcatenation = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_CONCAT, "-1", operationContext);
			}
		};
	...
	private void addBooleanOperators(JPopupMenu menu) {
		Action changeToAND = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_AND, "-1", operationContext);
			}
		};
	...
	private void addArithmeticOperations(JPopupMenu menu) {
		Action createAddition = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_ADDITION, "-1", forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_ADDITION, "-1", operationContext);
				}
			}
		};
		...
		Action createSubtraction = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_SUBTRACTION, "-1", forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_SUBTRACTION, "-1", operationContext);
				}
			}
		};
		createSubtraction
		...
		Action createMultiplication = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_MULTIPLICATION, "-1", forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_MULTIPLICATION, "-1", operationContext);
				}
			}
		};
		...
		Action createDivision = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_DIVISION, "-1", forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_DIVISION, "-1", operationContext);
				}
			}
		};
		...
		Action createIntDiv = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (isForHeader) {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_INTDIV, "-1", forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_INTDIV, "-1", operationContext);
				}
			}
		};
	...
	private void addComparison(JPopupMenu menu) {
		Action changeToLEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_LEQ, "-1", operationContext);
			}
		};
		...
		Action changeToLES = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_LES, "-1", operationContext);
			}
		};
		...
		Action changeToEQU = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_EQU, "-1", operationContext);
			}
		};
		...
		Action changeToNEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_NEQ, "-1", operationContext);
			}
		};
		...
		Action changeToGEQ = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_GEQ, "-1", operationContext);
			}
		};
		...
		Action changeToGRE = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				String expressionID = ((IDomainObjectUI) expression).getModelID();
				Services.getService().getController()
				        .createExpression(scopeModelID, expressionID, parentModelID, Expression.OPERATION_GRE, "-1", operationContext);
			}
		};
	...
	private void initChooseContentMenu() {
		...
		Action variableHasBeenChosen = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (isForHeader) {
					Services.getService().getController().createExpression(scopeModelID, "", parentModelID, Expression.VARIABLE, holdingType, forContext);
				} else {
					Services.getService().getController()
					        .createExpression(scopeModelID, "", parentModelID, Expression.VARIABLE, holdingType, operationContext);
				}
			}
		};
		...
		Action valueHasBeenChosen = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				if (isForHeader) {
					Services.getService().getController().createExpression(scopeModelID, "", parentModelID, holdingType, "-1", forContext);
				} else {
					Services.getService().getController().createExpression(scopeModelID, "", parentModelID, holdingType, "-1", operationContext);
				}
			}
		};
	...
	private void initChangeContentBtn() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!isComparison) {
					if (isComparisonEnabled) {
						...
						if (holdingType == Expression.DOUBLE_TYPE || holdingType == Expression.INTEGER_TYPE) {
							numberMenu.show(operationsBtn, operationsBtn.getWidth(), operationsBtn.getHeight());
						} else if (holdingType == Expression.STRING_TYPE) {
							stringMenu.show(operationsBtn, operationsBtn.getWidth(), operationsBtn.getHeight());
						} else if (holdingType == Expression.BOOLEAN_TYPE) {
		...
		action.putValue(Action.SMALL_ICON,
		        new ImageIcon(ExpressionHolderUI.class.getResource("/usp/ime/line/resources/icons/operations.png")));
	...
	private void expressionCreatedNormal(String holder, String id, String context) {
		if (holder.equals(parentModelID) && operationContext.equals(context)) {
			...
			if (expression != null)
				remove(expression);
			currentModelID = id;
			expression = Services.getService().getRenderer().paint(id, this.scopeModelID);
	...
	private void expressionCreatedFor(String holder, String id, String context) {
		if (holder.equals(parentModelID) && forContext.equals(context)) {
			...
			if (expression != null)
				remove(expression);
			currentModelID = id;
			expression = Services.getService().getRenderer().paint(id, scopeModelID);
			populatedExpressionHolder();
			add(expression, 0);
			if (expression instanceof VariableSelectorUI) {
				if (isEditing)
			...
			if (Services.getService().getViewMapping().getObject(parentModelID) instanceof CodeBaseUI) {
				// nessa atualização tenho que olhar o contexto
				if (lastExp != null && !"".equals(lastExp)) {
					Services.getService().getController()
	...
	private void expressionRestoredFromCleaningFor(String holder, String id, String context) {
		...
		if (holder.equals(parentModelID) && forContext.equals(context)) {
			...
			JComponent restoredExp = (JComponent) Services.getService().getViewMapping().getObject(id);
	...
	public void warningStateOn() {
		...
		if (Services.getService().getViewMapping().getObject(parentModelID) instanceof OperationUI) {
			((OperationUI) Services.getService().getViewMapping().getObject(parentModelID)).warningStateOn();
